...
base AS (
    SELECT
        u.acquisition_source AS channel,
        r.user_id,
        r.retained_w1,
        m.orders_7d,
        m.avg_aov,
...
-- Step 6: Channel-level retention (same value for both segments)
...
FROM base b
LEFT JOIN channel_retention cr
    ON b.channel = cr.channel

GROUP BY b.channel, user_segment, cr.w1_retention_rate
ORDER BY b.channel, user_segment;